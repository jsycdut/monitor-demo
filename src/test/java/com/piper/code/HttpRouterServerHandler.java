...
public class HttpRouterServerHandler extends SimpleChannelInboundHandler<HttpRequest> {
    ...
    private MXBeanJvmMetrics metrics;

    HttpRouterServerHandler(Router<String> router) {
        this.router = router;
        this.metrics = new MXBeanJvmMetrics();
    }
    ...
    public void channelRead0(ChannelHandlerContext ctx, HttpRequest req) {
        ...
        HttpResponse res = createResponse(req, router, this.metrics);
        flushResponse(ctx, req, res);
    }

    // Display debug info.
    private static HttpResponse createResponse(HttpRequest req, Router<String> router, JvmMetrics metrics) {
